            ...
            if (interaction.customId.startsWith('foraestoquealarme_')) {
                ...
                if (gggaaa.avisar !== undefined) {
                    if (!gggaaa.avisar.includes(interaction.user.id)) {
                        ...
                        if (status == 1) {
                            interaction.reply({ content: ` Você será notificado assim que for reabastecido o stock`, ephemeral: true })
                        } else {
                            interaction.update({ content: ` Você será notificado assim que for reabastecido o stock` })
                        }
                        ...
                        if (status == 1) {
                            interaction.reply({ content: `❗ Você já estava salvo para ser notificado, portanto foi retirado`, ephemeral: true })
                        } else {
                            interaction.update({ content: `❗ Você já estava salvo para ser notificado, portanto foi retirado` })
                        }
